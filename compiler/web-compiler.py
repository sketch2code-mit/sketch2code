...
def render_content_with_text(key, value):
    if FILL_WITH_RANDOM_TEXT:
        if key.find("btn") != -1:
            ...
            # might need a better way to replace different text sizes?its
            if key.find("big") != -1:
                value = value.replace(TEXT_PLACE_HOLDER, Utils.get_random_text(length_text=5, space_number=0))
            elif key.find("small") != -1:
                value = value.replace(TEXT_PLACE_HOLDER, Utils.get_random_text(length_text=15, space_number=3))
            else:
                value = value.replace(TEXT_PLACE_HOLDER, Utils.get_random_text(length_text=5, space_number=0))
        elif key.find("text") != -1:
            if key.find("big") != -1:
                value = value.replace(TEXT_PLACE_HOLDER, 
                                      Utils.get_random_text(length_text=10, space_number=1, with_upper_case=False))
            elif key.find("small") != -1:
                value = value.replace(TEXT_PLACE_HOLDER,
